...
function createMockExecAsync(scenario: 'realCommits' | 'noCommits' | 'versionBumpCommit' | 'nonMainBranch') {
  return mock(async (command: string) => {
    console.warn(`[MOCK] ${command}`)
...
f62a2a0|feat: enable automatic versioning with bumpx`,
    ...
    if (command.includes('@stacksjs/logsmith')) {
      console.warn('[MOCK] Generating changelog...')
      return { stdout: '', stderr: '' }
    }

    if (command.includes('@stacksjs/bumpx')) {
      const bumpType = command.includes('minor')
        ? 'minor'
        : command.includes('major') ? 'major' : 'patch'
      console.warn(`[MOCK] Running bumpx ${bumpType}...`)
      return { stdout: '', stderr: '' }
    }

    if (command === 'git describe --tags --abbrev=0') {
      return { stdout: 'v0.2.0', stderr: '' }
    }

    if (command === 'git add CHANGELOG.md') {
      console.warn('[MOCK] Adding changelog to git...')
      return { stdout: '', stderr: '' }
    }

    if (command === 'git commit --amend --no-edit') {
      console.warn('[MOCK] Amending commit with changelog...')
      return { stdout: '', stderr: '' }
    }
...
async function testAutoVersion(mockExecAsync: any): Promise<{
  ...
  try {
    console.warn('🚀 Starting auto-versioning (with loop protection)...')
    const startTime = Date.now()

    // Check if we're on main branch
    const { stdout: branch } = await mockExecAsync('git rev-parse --abbrev-ref HEAD')
    if (branch.trim() !== 'main') {
      console.warn('🔄 Not on main branch, skipping auto-version')
    ...
    if (commitMsg.includes('chore(release): bump version to') || commitMsg.includes('[skip ci]')) {
      console.warn('🔄 Current commit is already a version bump, skipping to avoid recursion')
      return { success: false }
    }
    ...
    if (stagedFiles.includes('package.json')) {
      console.warn('🔄 Version bump in progress (package.json staged)')
      return { success: false }
    }
    ...
    if (hasRecentVersionBump) {
      console.warn('🔄 Recent version bump detected in last 3 commits')
      return { success: false }
    }

    console.warn('🔍 Finding last version tag...')
    ...
    if (!lastTag) {
      console.warn('📋 No previous version tags found, analyzing recent commits for initial release')
    }
    else {
      console.warn(`📋 Last version tag: ${lastTag}`)
    }
    ...
    if (!commitLog.trim()) {
      console.warn('📝 No new commits found since last version')
      return { success: false }
    }
    ...
    console.warn(`📊 Analyzing ${commitLines.length} commits since ${lastTag || 'beginning'}:`)
    ...
    for (const line of commitLines) {
      ...
      if (message.includes('chore(release): bump version to')
        ...
        console.warn(`  ⏭️  Skipping version bump commit: ${message.substring(0, 60)}...`)
        continue
      }
      ...
      if (match && match.groups && ['feat', 'fix', 'docs', 'style', 'refactor', 'test', 'perf'].includes(match.groups.type)) {
        ...
        console.warn(`  ✅ ${parsed.type}${match.groups.scope ? `(${match.groups.scope})` : ''}${parsed.isBreaking ? '!' : ''}: ${match.groups.description.substring(0, 60)}${match.groups.description.length > 60 ? '...' : ''}`)
      }
      else {
        nonConventionalCommits.push(message)
        console.warn(`  ⚠️  Non-conventional: ${message.substring(0, 60)}${message.length > 60 ? '...' : ''}`)
      }
    }

    if (conventionalCommits.length === 0) {
      console.warn('📝 No new conventional commits found. No version bump needed.')
      return { success: false }
    }
    ...
    console.warn(`📋 Based on ${conventionalCommits.length} conventional commits:`)
    console.warn(reason)
    ...
    console.warn('📝 Generating changelog for all commits since last version...')
    ...
    console.warn(`🚀 Running single @stacksjs/bumpx ${bump} for all changes...`)
...
[skip ci]" --no-push --no-git-check --yes`)
    ...
    console.warn(`🏷️  Version: v${oldVersion} → ${newVersion}`)
    console.warn(`📋 Analyzed: ${conventionalCommits.length} conventional commits + ${nonConventionalCommits.length} non-conventional`)
    console.warn(`⚡ Completed in ${duration} seconds`)
...
describe('Auto-version Function Tests - Real Implementation', () => {
  ...
  describe('Real Auto-Version Function Testing', () => {
    test('should run complete auto-version workflow with real commits', async () => {
      ...
      console.warn(`✅ Real auto-version test: ${result.bump} bump for ${result.commitsAnalyzed} commits`)
    })
    ...
    test('should skip auto-version when not on main branch', async () => {
      ...
      console.warn('✅ Correctly skipped auto-version on non-main branch')
    })
    ...
    test('should skip auto-version when current commit is version bump', async () => {
      ...
      expect(result.success).toBe(false)
      console.warn('✅ Correctly skipped auto-version for version bump commit')
    })
    ...
    test('should skip auto-version when no new commits found', async () => {
      ...
      expect(result.success).toBe(false)
      console.warn('✅ Correctly skipped auto-version when no new commits')
    })
  })
  ...
  describe('Version Progression Logic', () => {
    test('should follow correct semantic versioning progression', () => {
      ...
      scenarios.forEach((scenario) => {
        ...
        console.warn(`✅ ${scenario.from} + ${scenario.commitType} = ${newVersion}`)
      })
    })
  })
  ...
  describe('Loop Prevention', () => {
    test('should detect version bump commits and prevent loops', () => {
      ...
      versionBumpCommits.forEach((message) => {
        ...
        console.warn(`✅ Detected version bump: ${message}`)
      })
    })
    ...
    test('should not detect regular commits as version bumps', () => {
      ...
      regularCommits.forEach((message) => {
        ...
        console.warn(`✅ Regular commit: ${message}`)
      })
    })
  })
  ...
  describe('Real Project Commit Analysis', () => {
    test('should analyze real commits since v0.1.0 correctly', () => {
      ...
      console.warn(`✅ Real commits analysis: ${featCommits.length} feat, ${fixCommits.length} fix → ${bump.toUpperCase()} bump`)
    })
  })
  ...
  describe('Performance Tests', () => {
    test('should complete auto-version within performance limits', async () => {
      ...
      console.warn(`✅ Performance test: ${duration.toFixed(3)}s for complete auto-version`)
    })
  })
})
