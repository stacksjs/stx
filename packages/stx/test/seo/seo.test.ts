...
// =============================================================================
// SEO advanced scenarios (from edge-case-bugs)
// =============================================================================

describe('SEO advanced scenarios', () => {
  it('should handle @seo with array keywords', () => {
    const template = `@seo({
      title: 'My Page',
  ...
  it('should handle injectSeoTags when description has special characters', () => {
    const html = '<html><head></head><body></body></html>'
    const result = injectSeoTags(html, { description: 'A "great" <site> & more' }, defaultOptions)
    expect(result).toContain('&amp;')
...
// =============================================================================
// SEO edge cases (from discovered-bugs)
// =============================================================================

describe('SEO edge cases (discovered)', () => {
  it('should handle @seo with empty config', () => {
    const r = processSeoDirective('@seo({})', {}, 'test.stx', defaultOptions)
    expect(r).toBe('')
  })
  ...
  it('should include multiple sitemaps in robots.txt', () => {
  ...
  it('should inject into head with attributes', () => {
    const html = '<html><head lang="en"></head><body></body></html>'
    const r = injectSeoTags(html, { title: 'Test' }, defaultOptions)
    expect(r).toContain('og:title')
  })

  it('should not duplicate title when title already exists in head', () => {
    const html = '<html><head><title>Existing</title></head><body></body></html>'
    const result = injectSeoTags(html, { title: 'New' }, defaultOptions)
    const titleCount = (result.match(/<title>/g) || []).length
    expect(titleCount).toBe(1)
  })
})
...
// =============================================================================
// @meta XSS prevention (from discovered-bugs)
// =============================================================================

describe('@meta XSS prevention', () => {
  it('should escape meta name containing angle brackets', () => {
    const r = processMetaDirectives("@meta('desc<script>', 'content')", {}, 'test.stx', defaultOptions)
    expect(r).not.toContain('<script>')
  })
})

// =============================================================================
// SEO Generation Edge Cases (from deep-edge-cases)
// =============================================================================

describe('SEO Generation Edge Cases (deep)', () => {
  ...
  it('processStructuredData with valid JSON-LD', () => {
    const template = `@structuredData({
      '@type': 'Organization',
      name: 'Example Corp',
  ...
  it('processSeoDirective with canonical URL containing special chars', () => {
    const template = `@seo({
      title: 'Test',
