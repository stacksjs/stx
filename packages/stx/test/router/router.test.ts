...
describe('STX Router', () => {
  ...
  describe('generateRouteTypes', () => {
    it('should generate route-types.d.ts', () => {
      ...
      // Augments the REAL package. This used to say `declare module
      // "stx/routes"` — a specifier nothing declares or imports, which made the
      // whole declaration inert (#1887).
      expect(content).toContain(`declare module '@stacksjs/stx'`)
      expect(content).toContain('interface KnownRoutes')
      // Without the trailing `export {}` the block is an ambient module
      // DECLARATION that replaces the package rather than augmenting it.
    ...
    it('should type catch-all params as a single string', () => {
      const routes = createRouter(TEMP_DIR)
      const outputDir = path.join(TEMP_DIR, '.stx')
      ...
      const content = fs.readFileSync(path.join(outputDir, 'route-types.d.ts'), 'utf8')
      // A catch-all is ONE capture that contains slashes, not an array:
      // `/blog/:slug*` against `/blog/2024/jan/hi` yields
      // `{ slug: '2024/jan/hi' }`. It was declared as `string[]`, which no
      // consumer could have used correctly.
