...
function patternToRegex(pattern: string): { regex: RegExp; params: string[] } {
  ...
  // Escape special regex characters except : * and ? (? is used for optional params)
  let regexStr = pattern.replace(/[.+^${}()|[\]\\]/g, '\\$&')

  // Handle optional params (/:param?) — the preceding / is also optional
...
export function createRouter(baseDir: string, options: RouterOptions = {}): Route[] {
  ...
  generateRouteTypes(routes, stxDir)
...
/**
 * Finds a custom error page for a given status code.
 * Checks for a specific code page (e.g., 500.stx) first, then falls back to generic error.stx.
 */
...
/**
 * Generates TypeScript route type declarations from discovered routes.
 * Writes to `.stx/route-types.d.ts` in the given output directory.
 */
export function generateRouteTypes(routes: Route[], outputDir: string): void {
  ...
  for (const route of routes) {
    ...
    for (const p of route.params) {
      // Catch-all params are string arrays
