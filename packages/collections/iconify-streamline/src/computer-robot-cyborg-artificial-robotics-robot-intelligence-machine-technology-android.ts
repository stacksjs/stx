import type { IconData } from '@stacksjs/iconify-core'

export const computerRobotCyborgArtificialRoboticsRobotIntelligenceMachineTechnologyAndroid: IconData = {
  "body": "<g fill=\"none\" stroke=\"currentColor\" stroke-linecap=\"round\" stroke-linejoin=\"round\"><path d=\"M12.5 9.5c0-3-2.46-5-5.5-5s-5.5 2-5.5 5v4h11ZM7 4.5v-4m-5.5 10h11m-7.5 0v3m4-3v3\"/><circle cx=\"5\" cy=\"8\" r=\".5\"/><circle cx=\"9\" cy=\"8\" r=\".5\"/></g>",
  "width": 14,
  "height": 14,
  "viewBox": "0 0 14 14"
}

export default computerRobotCyborgArtificialRoboticsRobotIntelligenceMachineTechnologyAndroid
