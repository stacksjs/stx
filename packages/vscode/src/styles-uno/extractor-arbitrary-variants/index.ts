...
import { restoreSkipCode, transformSkipCode } from '../integration/utils'
import { removeSourceMap } from './source-map'

export const quotedArbitraryValuesRE: RegExp = /(?:[\w&:[\]-]|\[\S{1,64}=\S{1,64}\]){1,64}\[\\?['"]?\S{1,64}?['"]\]\]?[\w:-]{0,64}/g

export const arbitraryPropertyRE: RegExp = /\[(\\\W|[\w-]){1,64}:[^\s:]{0,64}?("\S{1,64}?"|'\S{1,64}?'|`\S{1,64}?`|[^\s:]{1,64}?)[^\s:]{0,64}?\)?\]/g
