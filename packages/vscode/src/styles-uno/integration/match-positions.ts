...
import { arbitraryPropertyRE, quotedArbitraryValuesRE } from '../extractor-arbitrary-variants/index'

export async function isPug(uno: UnoGenerator, code: string, id = ''): Promise<{ pug: boolean, code: string }> {
...
export function getPlainClassMatchedPositionsForPug(codeSplit: string, matchedPlain: Set<string>, start: number): [number, number, string][] {
...
export async function getMatchedPositionsFromCode(
  ...
  id = '',
  options: GetMatchedPositionsOptions = {},
): Promise<(readonly [start: number, end: number, text: string])[]> {
