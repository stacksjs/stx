...
export function resolveId(id: string, importer?: string): string | undefined {
...
export function resolveLayer(id: string): string | undefined {
...
export function getCssEscaperForJsContent(view: string): (css: string) => string {
  if (!view)
...
export const HASH_PLACEHOLDER_RE: RegExp = /#--unocss-hash--\s*\{\s*content\s*:\s*\\*"([^\\"]+)\\*";?\s*\}/g
