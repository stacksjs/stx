/* eslint-disable no-cond-assign, regexp/no-unused-capturing-group */
...
export function findCssDefinitionForClass(document: vscode.TextDocument, className: string): CssDefinitionPosition | null {
  try {
    ...
    while ((styleMatch = styleBlockRegex.exec(text)) !== null) {
      ...
      const relativeText = styleContent
      let selectorMatch

      // Find the first match
      if ((selectorMatch = selectorRegex.exec(relativeText)) !== null) {
        // Calculate the document offset where this class is defined
        const absoluteOffset = styleStartOffset + selectorMatch.index
