...
```typescript
import { AbTestingIcon, AbacusIcon, AbjadArabicIcon } from '@stacksjs/iconify-mdi'

// Basic usage
const icon = AbTestingIcon()

// With size
const sizedIcon = AbTestingIcon({ size: 24 })

// With color
const coloredIcon = AbacusIcon({ color: 'red' })

// With multiple props
const customIcon = AbjadArabicIcon({
  size: 32,
  color: '#4a90e2',
  class: 'my-icon'
})
...
@js
  ...
  global.icons = {
    home: AbTestingIcon({ size: 24 }),
    user: AbacusIcon({ size: 24, color: '#4a90e2' }),
    settings: AbjadArabicIcon({ size: 32 })
  }
@endjs
...
```typescript
// Via color property
const redIcon = AbTestingIcon({ color: 'red' })
const blueIcon = AbTestingIcon({ color: '#4a90e2' })

// Via inline style
const greenIcon = AbTestingIcon({ style: 'color: green;' })

// Via CSS class
const themedIcon = AbTestingIcon({ class: 'text-primary' })
```
...
const icon24 = AbTestingIcon({ size: 24 })
const icon1em = AbTestingIcon({ size: '1em' })

// Set individual dimensions
const customIcon = AbTestingIcon({ width: 24, height: 32 })

// Only set height (width calculated from ratio)
const heightOnly = AbTestingIcon({ height: '1em' })
```
...
```typescript
const smallIcon = AbTestingIcon({ class: 'icon-small' })
const largeIcon = AbTestingIcon({ class: 'icon-large' })
```
...
```html
@js
  import { AbTestingIcon, AbacusIcon, AbjadArabicIcon, AbjadHebrewIcon } from '@stacksjs/iconify-mdi'

  global.navIcons = {
    home: AbTestingIcon({ size: 20, class: 'nav-icon' }),
    about: AbacusIcon({ size: 20, class: 'nav-icon' }),
    contact: AbjadArabicIcon({ size: 20, class: 'nav-icon' }),
    settings: AbjadHebrewIcon({ size: 20, class: 'nav-icon' })
  }
@endjs
...
```typescript
import { AbTestingIcon } from '@stacksjs/iconify-mdi'
...
const successIcon = AbTestingIcon({ size: 16, color: '#22c55e' })
const warningIcon = AbacusIcon({ size: 16, color: '#f59e0b' })
const errorIcon = AbjadArabicIcon({ size: 16, color: '#ef4444' })
```
...
1. **Use Component Functions**: Import component functions for cleaner code
   ```typescript
   // Recommended
   import { AbTestingIcon, AbacusIcon } from '@stacksjs/iconify-mdi'
   const icon = AbTestingIcon({ size: 24 })

   // Also works (data + renderIcon)
   import { abTesting, abacus } from '@stacksjs/iconify-mdi'
   import { renderIcon } from '@stacksjs/iconify-core'
   const icon = renderIcon(abTesting, { size: 24 })
   ```
...
3. **Cache Rendered Icons**: Render once and reuse multiple times
   ...
   @js
     import { AbTestingIcon } from '@stacksjs/iconify-mdi'
     global.icon = AbTestingIcon({ size: 24 })
   @endjs
...
4. **Use CSS for Theming**: Apply consistent styling through CSS classes
   ...
   ```typescript
   const icon = AbTestingIcon({ class: 'icon' })
   ```
