...
```typescript
import { 42Icon, 1001tracklistsIcon, 1and1Icon } from '@stacksjs/iconify-simple-icons'

// Basic usage
const icon = 42Icon()

// With size
const sizedIcon = 42Icon({ size: 24 })

// With color
const coloredIcon = 1001tracklistsIcon({ color: 'red' })

// With multiple props
const customIcon = 1and1Icon({
  size: 32,
  color: '#4a90e2',
  class: 'my-icon'
})
...
@js
  ...
  global.icons = {
    home: 42Icon({ size: 24 }),
    user: 1001tracklistsIcon({ size: 24, color: '#4a90e2' }),
    settings: 1and1Icon({ size: 32 })
  }
@endjs
...
```typescript
// Via color property
const redIcon = 42Icon({ color: 'red' })
const blueIcon = 42Icon({ color: '#4a90e2' })

// Via inline style
const greenIcon = 42Icon({ style: 'color: green;' })

// Via CSS class
const themedIcon = 42Icon({ class: 'text-primary' })
```
...
const icon24 = 42Icon({ size: 24 })
const icon1em = 42Icon({ size: '1em' })

// Set individual dimensions
const customIcon = 42Icon({ width: 24, height: 32 })

// Only set height (width calculated from ratio)
const heightOnly = 42Icon({ height: '1em' })
```
...
```typescript
const smallIcon = 42Icon({ class: 'icon-small' })
const largeIcon = 42Icon({ class: 'icon-large' })
```

## Available Icons

This package contains **3594** icons:
...
```html
@js
  import { 42Icon, 1001tracklistsIcon, 1and1Icon, 1dot1dot1dot1Icon } from '@stacksjs/iconify-simple-icons'

  global.navIcons = {
    home: 42Icon({ size: 20, class: 'nav-icon' }),
    about: 1001tracklistsIcon({ size: 20, class: 'nav-icon' }),
    contact: 1and1Icon({ size: 20, class: 'nav-icon' }),
    settings: 1dot1dot1dot1Icon({ size: 20, class: 'nav-icon' })
  }
@endjs
...
```typescript
import { 42Icon } from '@stacksjs/iconify-simple-icons'
...
const successIcon = 42Icon({ size: 16, color: '#22c55e' })
const warningIcon = 1001tracklistsIcon({ size: 16, color: '#f59e0b' })
const errorIcon = 1and1Icon({ size: 16, color: '#ef4444' })
```
...
1. **Use Component Functions**: Import component functions for cleaner code
   ```typescript
   // Recommended
   import { 42Icon, 1001tracklistsIcon } from '@stacksjs/iconify-simple-icons'
   const icon = 42Icon({ size: 24 })

   // Also works (data + renderIcon)
   import { 42, 1001tracklists } from '@stacksjs/iconify-simple-icons'
   import { renderIcon } from '@stacksjs/iconify-core'
   const icon = renderIcon(42, { size: 24 })
   ```
