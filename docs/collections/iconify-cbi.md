...
```typescript
import { 02tvIcon, 10PlayIcon, 17TrackIcon } from '@stacksjs/iconify-cbi'

// Basic usage
const icon = 02tvIcon()

// With size
const sizedIcon = 02tvIcon({ size: 24 })

// With color
const coloredIcon = 10PlayIcon({ color: 'red' })

// With multiple props
const customIcon = 17TrackIcon({
  size: 32,
  color: '#4a90e2',
  class: 'my-icon'
})
...
@js
  ...
  global.icons = {
    home: 02tvIcon({ size: 24 }),
    user: 10PlayIcon({ size: 24, color: '#4a90e2' }),
    settings: 17TrackIcon({ size: 32 })
  }
@endjs
...
```typescript
// Via color property
const redIcon = 02tvIcon({ color: 'red' })
const blueIcon = 02tvIcon({ color: '#4a90e2' })

// Via inline style
const greenIcon = 02tvIcon({ style: 'color: green;' })

// Via CSS class
const themedIcon = 02tvIcon({ class: 'text-primary' })
```
...
const icon24 = 02tvIcon({ size: 24 })
const icon1em = 02tvIcon({ size: '1em' })

// Set individual dimensions
const customIcon = 02tvIcon({ width: 24, height: 32 })

// Only set height (width calculated from ratio)
const heightOnly = 02tvIcon({ height: '1em' })
```
...
```typescript
const smallIcon = 02tvIcon({ class: 'icon-small' })
const largeIcon = 02tvIcon({ class: 'icon-large' })
```
...
```html
@js
  import { 02tvIcon, 10PlayIcon, 17TrackIcon, 2WayUplighterIcon } from '@stacksjs/iconify-cbi'

  global.navIcons = {
    home: 02tvIcon({ size: 20, class: 'nav-icon' }),
    about: 10PlayIcon({ size: 20, class: 'nav-icon' }),
    contact: 17TrackIcon({ size: 20, class: 'nav-icon' }),
    settings: 2WayUplighterIcon({ size: 20, class: 'nav-icon' })
  }
@endjs
...
```typescript
import { 02tvIcon } from '@stacksjs/iconify-cbi'
...
const successIcon = 02tvIcon({ size: 16, color: '#22c55e' })
const warningIcon = 10PlayIcon({ size: 16, color: '#f59e0b' })
const errorIcon = 17TrackIcon({ size: 16, color: '#ef4444' })
```
...
1. **Use Component Functions**: Import component functions for cleaner code
   ```typescript
   // Recommended
   import { 02tvIcon, 10PlayIcon } from '@stacksjs/iconify-cbi'
   const icon = 02tvIcon({ size: 24 })

   // Also works (data + renderIcon)
   import { 02tv, 10Play } from '@stacksjs/iconify-cbi'
   import { renderIcon } from '@stacksjs/iconify-core'
   const icon = renderIcon(02tv, { size: 24 })
   ```
...
4. **Use CSS for Theming**: Apply consistent styling through CSS classes
   ...
   ```typescript
   const icon = 02tvIcon({ class: 'icon' })
   ```
