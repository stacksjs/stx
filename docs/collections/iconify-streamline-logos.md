...
```typescript
import { 3dsMaxLogoIcon, 3dsMaxLogoBlockIcon, 3dsMaxLogoSolidIcon } from '@stacksjs/iconify-streamline-logos'

// Basic usage
const icon = 3dsMaxLogoIcon()

// With size
const sizedIcon = 3dsMaxLogoIcon({ size: 24 })

// With color
const coloredIcon = 3dsMaxLogoBlockIcon({ color: 'red' })

// With multiple props
const customIcon = 3dsMaxLogoSolidIcon({
  size: 32,
  color: '#4a90e2',
  class: 'my-icon'
})
...
@js
  ...
  global.icons = {
    home: 3dsMaxLogoIcon({ size: 24 }),
    user: 3dsMaxLogoBlockIcon({ size: 24, color: '#4a90e2' }),
    settings: 3dsMaxLogoSolidIcon({ size: 32 })
  }
@endjs
...
```typescript
// Via color property
const redIcon = 3dsMaxLogoIcon({ color: 'red' })
const blueIcon = 3dsMaxLogoIcon({ color: '#4a90e2' })

// Via inline style
const greenIcon = 3dsMaxLogoIcon({ style: 'color: green;' })

// Via CSS class
const themedIcon = 3dsMaxLogoIcon({ class: 'text-primary' })
```
...
const icon24 = 3dsMaxLogoIcon({ size: 24 })
const icon1em = 3dsMaxLogoIcon({ size: '1em' })

// Set individual dimensions
const customIcon = 3dsMaxLogoIcon({ width: 24, height: 32 })

// Only set height (width calculated from ratio)
const heightOnly = 3dsMaxLogoIcon({ height: '1em' })
```
...
```typescript
const smallIcon = 3dsMaxLogoIcon({ class: 'icon-small' })
const largeIcon = 3dsMaxLogoIcon({ class: 'icon-large' })
```
...
```html
@js
  import { 3dsMaxLogoIcon, 3dsMaxLogoBlockIcon, 3dsMaxLogoSolidIcon, 500pxLogo1Icon } from '@stacksjs/iconify-streamline-logos'

  global.navIcons = {
    home: 3dsMaxLogoIcon({ size: 20, class: 'nav-icon' }),
    about: 3dsMaxLogoBlockIcon({ size: 20, class: 'nav-icon' }),
    contact: 3dsMaxLogoSolidIcon({ size: 20, class: 'nav-icon' }),
    settings: 500pxLogo1Icon({ size: 20, class: 'nav-icon' })
  }
@endjs
...
```typescript
import { 3dsMaxLogoIcon } from '@stacksjs/iconify-streamline-logos'
...
const successIcon = 3dsMaxLogoIcon({ size: 16, color: '#22c55e' })
const warningIcon = 3dsMaxLogoBlockIcon({ size: 16, color: '#f59e0b' })
const errorIcon = 3dsMaxLogoSolidIcon({ size: 16, color: '#ef4444' })
```
...
1. **Use Component Functions**: Import component functions for cleaner code
   ```typescript
   // Recommended
   import { 3dsMaxLogoIcon, 3dsMaxLogoBlockIcon } from '@stacksjs/iconify-streamline-logos'
   const icon = 3dsMaxLogoIcon({ size: 24 })

   // Also works (data + renderIcon)
   import { 3dsMaxLogo, 3dsMaxLogoBlock } from '@stacksjs/iconify-streamline-logos'
   import { renderIcon } from '@stacksjs/iconify-core'
   const icon = renderIcon(3dsMaxLogo, { size: 24 })
   ```
...
3. **Cache Rendered Icons**: Render once and reuse multiple times
   ...
   @js
     import { 3dsMaxLogoIcon } from '@stacksjs/iconify-streamline-logos'
     global.icon = 3dsMaxLogoIcon({ size: 24 })
   @endjs
...
4. **Use CSS for Theming**: Apply consistent styling through CSS classes
   ...
   ```typescript
   const icon = 3dsMaxLogoIcon({ class: 'icon' })
   ```
