...
```typescript
import { AcceptIcon, AcceptMediumIcon, AccessLogoIcon } from '@stacksjs/iconify-fluent-mdl2'

// Basic usage
const icon = AcceptIcon()

// With size
const sizedIcon = AcceptIcon({ size: 24 })

// With color
const coloredIcon = AcceptMediumIcon({ color: 'red' })

// With multiple props
const customIcon = AccessLogoIcon({
  size: 32,
  color: '#4a90e2',
  class: 'my-icon'
})
...
@js
  ...
  global.icons = {
    home: AcceptIcon({ size: 24 }),
    user: AcceptMediumIcon({ size: 24, color: '#4a90e2' }),
    settings: AccessLogoIcon({ size: 32 })
  }
@endjs
...
```typescript
// Via color property
const redIcon = AcceptIcon({ color: 'red' })
const blueIcon = AcceptIcon({ color: '#4a90e2' })

// Via inline style
const greenIcon = AcceptIcon({ style: 'color: green;' })

// Via CSS class
const themedIcon = AcceptIcon({ class: 'text-primary' })
```
...
const icon24 = AcceptIcon({ size: 24 })
const icon1em = AcceptIcon({ size: '1em' })

// Set individual dimensions
const customIcon = AcceptIcon({ width: 24, height: 32 })

// Only set height (width calculated from ratio)
const heightOnly = AcceptIcon({ height: '1em' })
```
...
```typescript
const smallIcon = AcceptIcon({ class: 'icon-small' })
const largeIcon = AcceptIcon({ class: 'icon-large' })
```
...
```html
@js
  import { AcceptIcon, AcceptMediumIcon, AccessLogoIcon, AccessibiltyCheckerIcon } from '@stacksjs/iconify-fluent-mdl2'

  global.navIcons = {
    home: AcceptIcon({ size: 20, class: 'nav-icon' }),
    about: AcceptMediumIcon({ size: 20, class: 'nav-icon' }),
    contact: AccessLogoIcon({ size: 20, class: 'nav-icon' }),
    settings: AccessibiltyCheckerIcon({ size: 20, class: 'nav-icon' })
  }
@endjs
...
```typescript
import { AcceptIcon } from '@stacksjs/iconify-fluent-mdl2'
...
const successIcon = AcceptIcon({ size: 16, color: '#22c55e' })
const warningIcon = AcceptMediumIcon({ size: 16, color: '#f59e0b' })
const errorIcon = AccessLogoIcon({ size: 16, color: '#ef4444' })
```
...
1. **Use Component Functions**: Import component functions for cleaner code
   ```typescript
   // Recommended
   import { AcceptIcon, AcceptMediumIcon } from '@stacksjs/iconify-fluent-mdl2'
   const icon = AcceptIcon({ size: 24 })

   // Also works (data + renderIcon)
   import { accept, acceptMedium } from '@stacksjs/iconify-fluent-mdl2'
   import { renderIcon } from '@stacksjs/iconify-core'
   const icon = renderIcon(accept, { size: 24 })
   ```
...
3. **Cache Rendered Icons**: Render once and reuse multiple times
   ...
   @js
     import { AcceptIcon } from '@stacksjs/iconify-fluent-mdl2'
     global.icon = AcceptIcon({ size: 24 })
   @endjs
...
4. **Use CSS for Theming**: Apply consistent styling through CSS classes
   ...
   ```typescript
   const icon = AcceptIcon({ class: 'icon' })
   ```
