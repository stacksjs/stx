...
```typescript
import { AirplaneIcon, AlarmIcon, AlarmFillIcon } from '@stacksjs/iconify-f7'
...
// With color
const coloredIcon = AlarmIcon({ color: 'red' })

// With multiple props
const customIcon = AlarmFillIcon({
  size: 32,
  color: '#4a90e2',
  class: 'my-icon'
})
...
@js
  ...
  global.icons = {
    ...
    user: AlarmIcon({ size: 24, color: '#4a90e2' }),
    settings: AlarmFillIcon({ size: 32 })
  }
@endjs
...
```html
@js
  import { AirplaneIcon, AlarmIcon, AlarmFillIcon, AltIcon } from '@stacksjs/iconify-f7'

  global.navIcons = {
    home: AirplaneIcon({ size: 20, class: 'nav-icon' }),
    about: AlarmIcon({ size: 20, class: 'nav-icon' }),
    contact: AlarmFillIcon({ size: 20, class: 'nav-icon' }),
...
```typescript
import { AirplaneIcon } from '@stacksjs/iconify-f7'
...
const warningIcon = AlarmIcon({ size: 16, color: '#f59e0b' })
const errorIcon = AlarmFillIcon({ size: 16, color: '#ef4444' })
```
...
1. **Use Component Functions**: Import component functions for cleaner code
   ```typescript
   // Recommended
   import { AirplaneIcon, AlarmIcon } from '@stacksjs/iconify-f7'
