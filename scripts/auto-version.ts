...
async function checkProcessLock(): Promise<boolean> {
  try {
    ...
    if (stdout.trim() === 'exists') {
      ...
      const lockTime = Number.parseInt(timestamp.trim())
      const now = Date.now()
...
async function isVersionBumpContext(): Promise<boolean> {
  try {
    ...
    const versionBumpPatterns = [
      ...
      /^bump version/i, // Only if it starts with "bump version"
      /\[skip ci\]/i,
      /\[ci skip\]/i,
      /^v?\d+\.\d+\.\d+$/, // Version tag format
    ]
...
async function analyzeCommits(): Promise<{ bump: string, commits: number, reason: string } | null> {
  try {
    ...
    console.log(`🔍 Looking for version-bumping commits: feat (minor), fix (patch), BREAKING (major)`)
    ...
    for (const line of commitLines) {
      const [_hash, message] = line.split('|')
      ...
      if (match && match.groups) {
        ...
        // Only count commits that should trigger version bumps
        if (['feat', 'fix'].includes(type) || isBreaking) {
          conventionalCommits.push({
            type,
            breaking: isBreaking,
            message,
          })
          console.log(`  ✅ ${type}${isBreaking ? '!' : ''}: ${match.groups.description.substring(0, 50)}...`)
        }
        else {
          // Log non-versioning commits for transparency
          console.log(`  ℹ️  ${type}: ${match.groups.description.substring(0, 50)}... (no version bump)`)
        }
      }
    }

    if (conventionalCommits.length === 0) {
      console.log('📝 No commits requiring version bump found')
      return null
    }
    ...
    const hasFix = conventionalCommits.some(c => c.type === 'fix')
    ...
    if (hasBreaking) {
      ...
      console.log('📝 No version-bumping commits found')
      return null
    }
