...
| Form | Relationship |
|------|---------|
| `@if(cond) … @elseif … @else … @endif` | canonical Blade-style statement |
| `v-if` / `v-else-if` / `v-else` | Vue sugar → compiles to `@if` |
| `:if` / `:else-if` / `:else` | attribute form |
| `x-if` / `x-else-if` / `x-else` | Alpine sugar → same as `:if` |
...
28. **Global mountCallbacks Flush After Scope Processing**: The DOMContentLoaded handler flushes the global `mountCallbacks` array after processing all `[data-stx-scope]` elements. Previously, `mountCallbacks` was only flushed inside the `[data-stx]` loop (for setup function pages). Scripts that call `onMount()` with no registered scope for the current element push to the global array, so it must be flushed regardless of which code path registered the callbacks. A partial's `<script client>` is no longer one of them when its root is found: `wrapScopedInclude` (`includes.ts`) registers the scope before the body runs, so its `onMount`/`onDestroy` land on the scope, not on the global queues every `stx:load` drains (#1958). A rootless partial still uses the global queues.
...
40. **Dual reactive implementations**: stx ships TWO independent reactive systems — `packages/stx/src/signals-api.ts` (module-import path; used by composables, stores at module level, SSR helpers, tests run via `bun test` in Node) and the runtime template literal generated inside `packages/stx/src/signals.ts` (string-generated, injected into client pages, owns `window.stx.{state, derived, …}`). Both expose the same public surface (`state`, `derived`, `effect`, `batch`, `untrack`, `peek`, `isSignal`, `isDerived`, `onMount`, `onDestroy`) and must behave equivalently. The stx bundler rewrites `<script client>` imports so they destructure from `window.stx`; unbundled code lands at signals-api.ts. **Signals created in one impl are invisible to the other** — a `state()` returned by signals-api.ts cannot be `.set()`-ed from inside a client page's `<script>` (and vice versa). Parity is pinned by `test/reactivity/dual-impl-parity.test.ts`, which runs the same suite against both impls. When parity drifts, fix the lagging side — don't change only one. See `stacksjs/stx#1712` for the architectural rationale; one drift example already caught + fixed: `_isDerived` brand on derived signals (the stx.d.ts type had wrongly declared `_isSignal: true` for derived; corrected to match both impls). The same pattern extends to higher-level primitives that exist in both impls — currently `useCookie` is covered by `test/signals/use-cookie-parity.test.ts` (see `stacksjs/stx#1710`, which converted the composable from a Vue-style `CookieRef` to a Signal so both paths return the same shape). If you add a new primitive that lives in both `signals-api.ts`/`composables/` AND inside the runtime template literal, add a parity test next to it.

41. **CRITICAL: the client runtimes are TEMPLATE LITERALS — no backticks, not even in comments.** `generateSignalsRuntimeDev()` in `packages/stx/src/signals.ts` returns the entire ~6000-line client runtime as one template literal. `packages/router/src/client.ts` and `packages/stx/src/color-mode-boot.ts` do the same. Everything inside them is *string content*, not TypeScript, so:
    - **A backtick anywhere terminates the literal — including inside a `//` comment.** Writing ``// prefer `x` over `y` `` breaks the build. This has cost real debugging time four separate times; once it surfaced not as a syntax error but as a **Bun segfault** when a whole test directory ran, which took a long bisect to trace. Write comments in these files without backticks: say "prefer x over y" in plain words.
    - **Regex escapes must be doubled**: `\\s`, not `\s`. A single backslash is consumed by the literal, so `/\s+/` ships as `/s+/` — which silently matches the letter "s". That shipped once: `"docs".split(/s+/)` → `["doc"]`.
    - **`${` interpolates at generation time** unless escaped.
    - Don't add `console.log` — `test/signals/runtime-no-console-log.test.ts` pins the production build. And a debug log is not free: its **arguments evaluate even when the log is disabled**, so an unguarded read in one aborted hydration for a whole page (`bindIf`, fixed in `295d5904bb`) and another killed SPA navigation on every runtime-less page (the router, #1809).
    - After ANY edit to these files, run `bun test packages/stx/test/signals/runtime-parses.test.ts` — it parses both builds and names the offending line.
    - Tests that locate code inside the generated runtime should use `packages/stx/test-utils/runtime-surface.ts` rather than searching for assignment text. Five tests once anchored on the literal string `window.stx = {` and **returned an empty set** when it changed, which would have turned a drift guard into a test that passed because it checked nothing.

42. **Components the layout renders persist across same-layout navigation (#1958).** A component outside the router's container (a nav, a sidebar, a sign-in sheet) keeps its root, its bindings and its instance when a navigation keeps the layout. Its script must therefore NOT run again, or it sets up a second instance its markup never sees (state set afterwards never renders, listeners pile up). The pieces:
    - **The stamp.** A script that sets up one component instance says so. `data-stx-owner="<scope id>"` names the scope root it binds (component tags and islands in `utils.ts`, and in `includes.ts` only the script whose id the partial's root carries, and only when that root is an element: a root that is a component tag renders without the id, and an owner no element carries is read as one that stayed, so that partial's script would stop running on navigation; `rendersAsElement` in `component-renderer.ts` decides). `data-stx-instance="<uid>"` marks a component with no scope root (no signals; `processClientScript`'s `instanceId`, passed only by the component renderer). Page setups, route params, the component factory prelude, the reactive bridge and the runtime are never stamped: they run for every page.
    - **Fragments** leave out stamped scripts outside the container (`extractContainerContent` in `app-shell.ts`; `appendOutsideMainScript` in bun-plugin `serve.ts`, only when a container was found). Navigation swaps a fragment in only when the layout is unchanged; `submitForm` still swaps a fragment response without that check, so a form landing on another layout is not covered.
    - **The router** skips a `data-stx-owner` script whose root stayed on the page (gone, or present and bound; e9a59e7bb4), on both paths, and on the full-document path skips a `data-stx-instance` script collected from outside the container, and collects code only there (an island's `type="stx/island"` script waits for its trigger). On a layout change it runs `_cleanupContainer(document.body)` before swapping the body, so the outgoing chrome's components are destroyed there, and every script in the new body runs.
    - **Lifecycle** has to be the instance's own for this to work: an onDestroy registered inside a running mount callback (a composable in `onMount`) goes to that flush's sink (`activeDestroySink` in the runtime's `runMountCallbacks`), not to the global queue that every `stx:load` drains. `stx.mount`/`mountEl` suspend that sink while their own setup runs, since they collect the setup's hooks off the global queues for their root: a component mounted from inside another's `onMount` keeps its own teardown. One registered later, after an await or in a timer, by a composable or by a partial's `onDestroy`, still goes global.
    - To react to route changes, a layout component listens for `stx:navigate`. Tests: `test/router/persistent-layout-component.test.ts` drives the whole pipeline (processDirectives, the real runtime and router, fragment and whole-document servers) through `test/router/spa-harness.ts`.
