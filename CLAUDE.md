...
12. **Script Unification**: `<script>` and `<script client>` are the same (both client-side). Only `<script server>` runs on the server. This was fixed in component rendering (`utils.ts`) — previously bare `<script>` was incorrectly treated as server-side.
